...
const ROLES = ["manager", "salesperson", "basic"];

export default function RoleSelection({ currentRole, userId }: { currentRole: Role, userId: number }) {
  const [role, setRole] = useState<Role>(currentRole);
  const { toast } = useToast();

  async function handleRoleSelection(newRole: Role) {
    //lets save the previous role if this goes to sh*t, we can revert
    ...
    <>
      <div className="relative z-20 bg-transparent dark:bg-dark-2">
        <select
          value={role}
          onChange={(e) => {
            handleRoleSelection(e.target.value as Role);
          }}
        ...
        >
          {ROLES.map((role) => (
            <option key={role} value={role} className="text-dark-6 uppercase">
              {role}
            </option>
          ))}
        </select>
